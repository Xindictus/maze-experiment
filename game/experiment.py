...
# Game utility file
from game.game_utils import get_agent_only_action, get_distance_traveled, get_env_action
...
class Experiment:
    def __init__(
        ...
        if self.config["Experiment"]["mode"] != "human":
            ...
            if self.args.ppr:
                self.second_agent.load_models()
                self.probablistic_policy_reuse = [
                    0.7,
                    0.55,
                    0.4,
                    0.25,
    ...
    def human_play(self, participant_name):
        ...
        self.human_replay_buffer.save_buffer(
            os.path.join(
                "results", self.mode, "buffer", self.participant_name
            ),
            file_name,
    ...
    def maze_only_human(
        ...
        for i_game in range(max_games):
            ...
            while True:
                ...
                game_agent_states.append(new_row)
                self.human_replay_buffer.add(
                    observation,
                    action_pair[0],
                    reward,
                    observation,
                    done,
                    "human_" + str(block_name) + "_" + str(i_game),
                )
    ...
    def get_agent_action(
        ...
        if not self.second_human and enemy_status == "agent":
            agent_action, argmax_action = self.compute_agent_action(
                prev_observation, enemy_status, game_mode
            )
            if game_mode == "train":
            ...
            agent_action, argmax_action = self.compute_agent_action(
                prev_observation, "agent", game_mode
            )
            if game_mode == "train":
    ...
    def compute_agent_action(self, observation, status, game_mode):
        if status == "agent":
            ...
            if self.args.ppr and game_mode == "train":
                ...
                if (
                    random_chance
                    < self.probablistic_policy_reuse[self.current_block]
                ):
    ...
    def offline_grad_updates_session(self, i_game, block_number):
        ...
        if self.update_cycles > 0:
            grad_updates_duration = self.grad_updates(
                self.update_cycles, block_number + 1
            )

    ...
    def schedule(
        ...
        if config["Experiment"]["online_updates"]:
            online_updates = (
                ...
                * (
                    max_games
                    - config["Experiment"][mode]["start_training_step_on_game"]
                )
            )
        ...
        if config["Experiment"]["scheduling"] == "descending":
            ...
            print("total_update_cycles: ", total_update_cycles)
            print("update_cycles: ", update_cycles)
            print("max_games: ", max_games)
            print("update_interval: ", update_interval)
            print(
                "ceil(max_games / update_interval): ",
                math.ceil(max_games / update_interval),
            )

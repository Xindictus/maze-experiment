    ...
    def sample(self,block_nb):
        ...
        if self.args.dqfd:
            ...
            if int(self.batch_size*(1 - splits[block_nb])) > 0 :
                self_data = random.sample(self.memory,k=int(self.batch_size*(1 - splits[block_nb])))

                states = torch.from_numpy(np.stack([e.state for e in self_data if e is not None])).float().to(self.device)
                actions = torch.from_numpy(np.vstack([e.action for e in self_data if e is not None])).float().to(self.device)
                rewards = torch.from_numpy(np.vstack([e.reward for e in self_data if e is not None])).float().to(self.device)
                next_states = torch.from_numpy(np.stack([e.next_state for e in self_data if e is not None])).float().to(self.device)
                dones = torch.from_numpy(np.vstack([e.done for e in self_data if e is not None]).astype(np.uint8)).float().to(self.device)
            ...
            if self.batch_size*splits[block_nb] > 0:
                ...
                if int(self.batch_size*(1 - splits[block_nb])) > 0 :
                    ...
                    rewards = torch.from_numpy(np.vstack([e.reward for e in dem_batch if e is not None])).float().to(self.device)
                    next_states = torch.from_numpy(np.stack([e.next_state for e in dem_batch if e is not None])).float().to(self.device)
                    dones = torch.from_numpy(np.vstack([e.done for e in dem_batch if e is not None]).astype(np.uint8)).float().to(self.device)
        else:       
